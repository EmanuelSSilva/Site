document.addEventListener("DOMContentLoaded", function () {
	//JavaScript, pegar o evento de submit do formulário para evitar o recarregamento da página.
	...
	formulario.addEventListener("submit", async function (evento) {
		...
		// Obter o valor digitado pelo usuário no campo de texto.
		...
		// Exibir um indicador de carregamento enquanto a requisição está sendo processada.
		mostrarCarregamento(true);

		// Faz requisição HTTP (POST) para a API do n8n, enviando o texto do formulário no corpo da requisição em formato JSON.
		try {
			const resposta = await fetch("https://silvaemanuels.app.n8n.cloud/webhook/Gerador-backgroud", {
				method: "POST",
				headers: {
	...
	//Receber a resposta da API do n8n (esperando um JSON com o código HTML/CSS do background).
	// Se a resposta for válida, exibir o código HTML/CSS retornado na tela:
	...
	// Remover o indicador de carregamento após o recebimento da resposta.
});
